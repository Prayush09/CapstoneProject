  ...
  async function storePreferences(subcategory){
    ...
    if(preferencesOfUser.length < 10)
    {
      if(preferencesOfUser.includes(subcategory)){
        ...
        setPreferencesOfUser([...preferencesOfUser, subcategory]);
      }
    }
    else{
      const preferencesData = preferencesOfUser.reduce((acc, pref, index) => {
          acc[`pref${index+1}`] = pref;
          return acc;
      }, {});
      await axios.post('http://localhost:3000/preferences', JSON.stringify(preferencesData));
